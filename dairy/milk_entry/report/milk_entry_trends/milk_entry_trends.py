    ...
    if filters.get("group_by"):
        ...
        data1 = frappe.db.sql(""" select %s from `tab%s` t1
                ...
                """ % (query_details, conditions["trans"],
					   # conditions["trans"], conditions["addl_tables"],
					   "%s",
                       posting_date, "%s", "%s", conditions.get("addl_tables_relational_cond"), cond,
        ...
        for d in range(len(data1)):
            ...
            for i in range(len(row)):
                ...
                row1 = frappe.db.sql(""" select %s , %s from `tab%s` t1
                            ...
                            and t1.docstatus = 1 and %s = %s and %s = %s %s %s
                        """ %
                                     (sel_col, conditions["period_wise_select"], conditions["trans"],"%s", posting_date, "%s",
                                      "%s", sel_col,
        ...
        data = frappe.db.sql(""" select %s from `tab%s` t1
                ...
                """ %
                             (query_details, conditions["trans"],
							  # conditions["trans"], conditions["addl_tables"],
                              "%s", posting_date, "%s", "%s", cond,
                              conditions.get("addl_tables_relational_cond", ""), conditions["group_by"]),
                             (filters.get("company"), year_start_date, year_end_date), as_list=1)
