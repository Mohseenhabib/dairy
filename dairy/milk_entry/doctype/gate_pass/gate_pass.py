...
class GatePass(Document):
	...
	def before_submit(sales):
		...
		if frappe.db.get_single_value("Dairy Settings", "leakage_calculated_on") == "Gate Pass":
			if frappe.db.get_single_value("Dairy Settings", "leakage_percentage") and frappe.db.get_single_value(
					"Dairy Settings", "leakage_qty"):
		...
		# dist_item = frappe.db.sql(""" select distinct(item_code) from `tabMerge Gate Pass Item` where parent = %(name)s """,{'name':sales.name})
		# for itm in dist_item:
		if frappe.db.get_single_value("Dairy Settings", "crate_reconciliation_based_on") == "Gate Pass":
			dist_cratetype = frappe.db.sql(""" select distinct(crate_type) from `tabMerge Gate Pass Item` 
			where parent = %(name)s""",{'name':sales.name})
			for crate in dist_cratetype:
				dist_warehouse = frappe.db.sql(""" select distinct(warehouse) from `tabMerge Gate Pass Item` 
							where parent = %(name)s and crate_type = %(crate_type)s """,
							{'name': sales.name,'crate_type':crate})
				for warehouse in dist_warehouse:
					sums = frappe.db.sql(""" select sum(out_crate) as crate, sum(in_crate) as crate_ret, sum(damaged_crate) as damaged_crate
					 from `tabMerge Gate Pass Item` where crate_type = %(crate)s and parent = %(name)s 
					 and warehouse = %(warehouse)s""",{'crate':crate,'name':sales.name,'warehouse':warehouse},as_dict=1)
					print("sum ********",sums)
					print("crate ***",crate)
					...
					openning_cnt = frappe.db.sql(""" select count(*) from `tabCrate Log`  where crate_type = %(crate)s and source_warehouse = %(warehouse)s
									 and company = %(company)s and  docstatus = 1	 order by date desc  """,{'crate': crate, 'warehouse': warehouse,
											  'company': sales.company}, as_dict=1)
					...
					if openning_cnt[0]['count(*)'] > 0:
						...
						company = %(company)s and  docstatus = 1 order by date desc limit 1 """,{'crate':crate,'warehouse':warehouse,'company':sales.company},as_dict=1)
						print("------------",openning)
						...
						# log.crate_opening = sums[0]['crate'] + sums[0]['crate_ret']
						# log.crate_balance = sums[0]['crate'] + sums[0]['crate_ret']
						# sales.append("crate_summary", {
						# 	"crate_opening": sums[0]['crate'] + sums[0]['crate_ret'],
						# 	"crate_issue": sums[0]['crate'],
						# 	"crate_return": sums[0]['crate_ret'],
						# 	"crate_balance": sums[0]['crate'] + sums[0]['crate_ret']
						# })
					log.save()
					log.submit()
...
def merge_items(doc_name):
	...
	# doc.total_crate_return = total_crate_return
	doc.save()
